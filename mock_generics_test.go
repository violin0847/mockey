		...
		PatchConvey("args-type", func() {
			...
			MockGeneric(target).To(
				func(info GenericInfo, t1 int, t2 float32, t3 string) (r1 chan (int), r2 []byte, r3 struct{ _ int }) {
					convey.So(info.TypeOfUsedArgN(0), convey.ShouldEqual, reflect.TypeOf(t1))
					convey.So(info.TypeOfUsedArgN(1), convey.ShouldEqual, reflect.TypeOf(t2))
					convey.So(info.TypeOfUsedArgN(2), convey.ShouldEqual, reflect.TypeOf(t3))
					convey.So(info.TypeOfUsedArgN(3), convey.ShouldEqual, reflect.TypeOf(r1))
					convey.So(info.TypeOfUsedArgN(4), convey.ShouldEqual, reflect.TypeOf(r2))
					convey.So(info.TypeOfUsedArgN(5), convey.ShouldEqual, reflect.TypeOf(r3))
					return
				}).Build()
			target(1, 2, "3")
		})
